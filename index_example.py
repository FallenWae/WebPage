...
from pages import obs_alarm, op_alarm, cl_alarm, prise_alarm, pfall_alarm, vcb_alarm, obcu_alarm, ebrake_alarm

alarms_list = ['obs_alarm', 'op_alarm', 'cl_alarm', 'prise_alarm', 'pfall_alarm', 'vcb_alarm', 'obcu_alarm', 'ebrake_alarm']
    ...
    children=[
           ...
           dbc.DropdownMenu(
            children=[
                ...
                dbc.DropdownMenuItem("VCB Alarm", href="/pages/vcb_alarm"),
                dbc.DropdownMenuItem("OBCU Alarm", href="/pages/obcu_alarm"),
                dbc.DropdownMenuItem("Emergency Brake Alarm", href="/pages/ebrake_alarm"),
            ],
            nav=True,
            in_navbar=True,
            label="Alarms",
...
sidebar = html.Div(
    ...
    dbc.Nav(
        [  
            ...
            dbc.NavLink("VCB", href="/pages/vcb_alarm", id = 'vcb_alarm_link'),
            dbc.NavLink("OBCU", href="/pages/obcu_alarm", id = 'obcu_alarm_link'),
            dbc.NavLink("Emergency Brake", href="/pages/ebrake_alarm", id = 'ebrake_alarm_link'),
        ],
        vertical=True,
        pills=True,
   ...
   id = 'sidebar', 
   style = {    "position": "fixed", "top": 62.5, "left": 0, "bottom": 0, "width": "16rem", "height": "100%", "z-index": 1, "overflow-x": "hidden", "transition": "all 0.5s", "padding": "0.5rem 1rem", "background-color": "#f8f9fa"}, 
)
...
app.layout = html.Div([
    ...
    navbar,
    sidebar,
    html.Div(id='page-content', children=[], style = {"transition": "margin-left .5s", "margin-left": "18rem", "margin-right": "2rem", "padding": "2rem 1rem", "background-color": "#f8f9fa",} )
])
...
def display_page(pathname):
    ...
    if pathname == '/pages/vcb_alarm':
        try:
            return vcb_alarm.layout
        except:
            return html.Div([
            html.H1('404 Not Found', className = 'text-danger'),
            html.Hr(),
            html.P(f'No VCB Alarms from {StartDate} to {EndDate}'),
        
         ],
        )

    if pathname == '/pages/obcu_alarm':
        try:
            return obcu_alarm.layout
        except:
            return html.Div([
            html.H1('404 Not Found', className = 'text-danger'),
            html.Hr(),
            html.P(f'No OBCU Alarms from {StartDate} to {EndDate}'),
        
         ],
        )

    if pathname == '/pages/ebrake_alarm':
        try:
            return ebrake_alarm.layout
        except:
            return html.Div([
            html.H1('404 Not Found', className = 'text-danger'),
            html.Hr(),
            html.P(f'No Emergency Brake Alarms from {StartDate} to {EndDate}'),
